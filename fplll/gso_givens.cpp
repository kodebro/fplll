...
template <class ZT, class FT> void MatGSOGivens<ZT,FT>::apply_givens_operations(int row){
  ...
  /*} else {
	  for(int i = 0; i < row; i++) {
		  ...
		  if (i == lazy_row_position) {
		  	for(int j = end_lazy_row; j>=start_lazy_row; j--) {
		   		// maybe do a check if ops_s(i,j) is zero?
		    	virtual_givens_rotation(row,j, lazy_row_c(0,j), lazy_row_s(0,j));
...
{
  throw std::runtime_error("Use of SWAP!");
  /*
